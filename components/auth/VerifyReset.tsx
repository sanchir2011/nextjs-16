...
import { resetPassword, forgotCheckId } from '@/lib/request'
import { motion } from 'framer-motion'
import { useRouter } from 'next/navigation'
import { toast } from '@/lib/toast'
import { CircleX, HomeIcon } from 'lucide-react'
...
export default function VerifyReset(){
  ...
  useEffect(() => {
    if(email && id) {
      const verify = async () => {
        const response = await forgotCheckId({ email, id })
  ...
  const handleResetPassword = async (e: any) => {
    ...
    if(!id) return toast.error('ID хоосон байна')
    ...
    setBtnLoading(true)
    const response = await resetPassword({ email, password, id })
    if(!response) {
      setBtnLoading(false)
      ...
      { !loading && (
        valid ? (
          ...
          <div className='flex flex-col gap-4 items-center justify-center'>
            ...
            <motion.div className='text-center text-xl font-semibold text-foreground font-gilroy' initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}>
              Код хүчингүй болсон байна 😢
            </motion.div>
            <motion.div className='grow-0 mt-4' initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1 }}>
              <Button onClick={() => router.push('/login')} icon={HomeIcon}>
                Нэвтрэх
              </Button>
            </motion.div>
          </div>
